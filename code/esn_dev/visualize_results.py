...
def animate_comparison(targets,predictions,mask=False,filepath='comparison.mp4',fps=24,dpi=300, v=(None,None)):
    targets    = targets.copy()
    predictions= predictions.copy()
    if v[0] is None:
        #min/max
        vmin = min(targets.min(),predictions.min())*4/5
        vmax = max(targets.max(),predictions.max())*4/5
    else:
        vmin = v[0]
        vmax = v[1]
    ...
    fig.tight_layout()
    im1 = ax1.imshow(targets[0,:,:],cmap='viridis',origin='lower',vmin=vmin,vmax=vmax)
    im2 = ax2.imshow(predictions[0,:,:],cmap='viridis',origin='lower',vmin=vmin,vmax=vmax)
...
def animate_this(anim_data, filepath='animation.mp4',fps=24,dpi=150):
    ...
    def animate(i):
        ...
        return (im,)
    fig, ax = plt.subplots()
    vmin = anim_data.min()*1/2
    vmax = anim_data.max()*1/2
...
def MSE_over_time(targets, predictions, subplot_kw=None):
    ...
    ax.plot(
        ...
        mec='red',
        mfc='red',
        label='MSE Over Time (markers represent individual MSE points)'  # Updated legend label
    )
    ...
    ax.legend(loc='upper left')  # Add legend in the upper left corner
