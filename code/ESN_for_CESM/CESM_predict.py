...
from esn_dev.visualize_results_anom import MSE_over_time, animate_comparison
...
def train_and_predict_2d(data, lon, lat, unit, specs,
                             savedir='tmp',
                             spectral_radius=1.5,
    ...
    Params:
        ...
        lon:         (M, N) longitude array
        lat:         (M, N) latitude array
        unit:        str - unit of variable for plotting 
    ...
    # --- Building Dyn. Sys ---
    print("Starting Building Dyn. Sys. ...")
    ...
    # --- Evolution & Harvest of dyn. sys. ---
    print("Starting transient Evolution of dyn. sys. ...")

    start_trans = time()
    # compute final state after transient warmup of hidden state
    # Use only Ntrans-part of training set, no optimization on this.
    # print(f"esn size is {esn.shape}")
    # print(f"train_input size is {train_inputs[:Ntrans].shape}")
    ...
    print("Starting harvest of echo states ...")
    ...
    # --- PCA dim. reduction ---
    print("Starting PCA dim. reduction ...")
    ...
    print("Starting to create Readout Matrix using lstsq")
    ...
    # Sum over time
    time_int_spat_error = np.cumsum(pred_error, axis=0) # shape: (lon, lat)
    ...
    if plot_prediction and folder is not None:
        ...
        mse_fig.savefig(f'{folder}/MSE_plot.png', bbox_inches='tight')
